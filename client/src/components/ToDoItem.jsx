export function TodoItem({ complete, id, text, toggleTodo, deleteTodo }) {
    return (
        <li className="flex justify-evenly gap-3 bg-sky-900 w-full py-1 px-4 rounded-xl">
            <input type="checkbox" checked={complete} onChange={e => toggleTodo(id)} className="accent-sky-950 hover:accent-white"/>
            <label className="flex cursor-pointer select-none items-center">{text}</label>
